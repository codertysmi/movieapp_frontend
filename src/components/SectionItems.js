...
function SectionItems(props) {
  ...
  function handleClick(e) {
    ...
    var media_type = ""
    if(!props.item.media_type && props.media_type){
      media_type = "movie"
    }else{
      media_type = props.item.media_type
    }
    navigate(`/${media_type}/${props.item.id}`);
}

  return (
    <Grid2 sx={12} s={4} md={4} lg={1.714} sx={{display: "flex", flexDirection: "column", rowGap: "10px", width: "100%"}}>
        <Paper onClick={handleClick} sx={{ minWidth: "100%", height: "300px", position: "relative", cursor: "pointer"}}>
        ...
        <div style={isMobile ? {display: "flex", alignItems: "left", flexDirection: "column", justifyContent: "left", alignContent: "left", cursor: "pointer"} : {cursor: "pointer", display: "flex", justifyContent: "left", alignContent: "left", alignItems: "left", flexDirection: "column", justifyContent: "space-between"}}>
